...
var active, addCommas, bbContext, bbFocus, canvasHeight, canvasWidth, clicked, constructHourlyObject, contextFrameMask, contextInnerFrame, contextOuterFrame, drawVisualization, focusArea, focusFrame, focusLine, focusXAxis, focusXScale, focusYAxis, focusYScale, mapX, mapY, margin, padding, path, projection, reset, sumScaledown, svg, tooltipOffset;
...
projection = d3.geo.albersUsa().scale(900).translate([mapX, mapY]);
...
drawVisualization = function(states, stations) {
  ...
  focusFrame.append("text").attr("class", "x label focus").attr("text-anchor", "end").attr("x", bbFocus.width - padding.labelX).attr("y", bbFocus.height - padding.labelY).text("Hour");
  focusFrame.append("text").attr("class", "y label focus").attr("text-anchor", "end").attr("y", padding.labelY).attr("dy", ".75em").attr("transform", "rotate(-90)").text("GHI (lx)");
  ...
  updateFocus = function(d) {
    ...
    focusFrame.select(".area.focus").datum(dataset).attr("d", focusArea);
    return focusFrame.select(".line.focus").datum(dataset).attr("d", focusLine);
  };
